...
def sheet_to_suite(root_topic):
    ...
    suite.details = root_topic.get('note')
    suite.sub_suites = []
...
def parse_testsuite(suite_dict):
    ...
    testsuite.details = suite_dict.get('note')
...
def is_testcase_topic(case_dict):
    """A topic with a priority maker, or no subtopic, indicates that it is a testcase"""
...
def parse_a_testcase(case_dict, parent):
    ...
    testcase.result = get_test_result(case_dict.get('makers'))
...
def get_execution_type(topics):
    labels = []
    for topic in topics:
        # xmindparser 统一返回 labels（列表），兼容旧版 label（字符串）
        lbs = topic.get('labels', [])
        if isinstance(lbs, list):
            labels.extend(lbs)
        elif isinstance(lbs, str):
            labels.append(lbs)
...
def get_priority(case_dict):
    """Get the topic's priority（equivalent to the importance of the testcase)
       获取优先级数字的整数
    """
    if isinstance(case_dict.get('makers'), list):
        for maker in case_dict['makers']:
            if maker.startswith('priority'):
                return int(maker[-1])
...
def gen_testcase_preconditions(topics):
    notes = [topic.get('note') for topic in topics]
